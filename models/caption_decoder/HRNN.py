...
class HRNN(nn.Module):
    def __init__(self, opt):
        ...
        opt.event_context_dim = opt.clip_context_dim if opt.event_context_dim == 0 else opt.event_context_dim
        self.eposide_rnn = nn.LSTM(self.rnn_size + opt.event_context_dim,
                                                              self.rnn_size, 1, bias=False,
    ...
    def forward(self, event, clip, clip_mask, seq, event_seq_idx, event_feat_expand=False):
        ...
        for idx in range(eseq_len):
            ...
            sent_rnn_input = torch.cat((last_sent_state, event_idx), 1).unsqueeze(0)
            _, para_state = self.eposide_rnn(sent_rnn_input, para_state)
            state = para_state
...
class ShowAttendTellModel(HRNN):
    def __init__(self, opt):
