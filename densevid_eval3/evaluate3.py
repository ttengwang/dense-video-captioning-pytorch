    ...
    def import_prediction(self, prediction_filename):
        ...
        for vid_id in submission['results']:
            ...
            if len(all_proposals):
                proposal_scores = [proposal.get('proposal_score', 0) for proposal in all_proposals]
                ...
                for p in range(len(all_proposals)):
                    if all_proposals[p].get('proposal_score', 0)>=score_threshold:
...
def eval_score(json_path, onlyMeteor, onlyRecallPrec, topN=1000, reference=None):
    class Empty():
        pass
    args = Empty()
    ...
    args.verbose = not onlyMeteor
    args.onlyRecall = onlyRecallPrec
    if reference is None:
        args.references = ['data/captiondata/val_1.json', 'data/captiondata/val_2.json']
    else:
